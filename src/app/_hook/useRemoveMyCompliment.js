...
export function useRemoveMyCompliment(compliment_id) {
    ...
    const removeCompliment = async () => {
        ...
        try {
          ...
          const Likes = await getDocs(query(collection(db, "likes"), where("compliment_id", "==", compliment_id)));
          Likes.forEach(async (doc) => {
            await deleteDoc(doc.ref);
          });

          const Comments = await getDocs(query(collection(db, "comments"), where("compliment_id", "==", compliment_id)));
