      ...
      <Card className=" bg-white bg-opacity-40 my-4 shadow-none">
        <div className="text-left flex flex-col gap-4">
          ...
          <div>
            ...
            <RatingButton
              readOnly={true}
              ratingValue={complimentInfo.compliment_rating}
            />
          </div>
          <div>
            <h5 className={mPlus1Bold.className + " text-xs"}>その内容</h5>
            <div className={mPlus1.className}>{complimentInfo.body}</div>
          </div>
          <div>
            <h5 className={mPlus1Bold.className + " text-xs"}>思ったこと</h5>
            <div className={mPlus1.className}>{complimentInfo.thoughts}</div>
          </div>
          <Tags readonly={true} tags={complimentInfo.tags} />
          <div className="mx-auto flex justify-between items-center w-44">
            <LikeButton
              isLiked={complimentInfo.isLiked}
              countOfLikes={complimentInfo.count_of_likes}
              complimentId={complimentInfo.id}
            />
            <CommentButton
              countOfComment={complimentInfo.count_of_comments}
              complimentRoute={`/compliments/${complimentInfo.id}`}
            />
          </div>
        </div>
